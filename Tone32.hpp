...
class Tone32{
  ...
  public:
     ...
     */
    Tone32(uint8_t outputPin, uint32_t frequency, uint8_t resolution){
        setupOutputs(outputPin, frequency, resolution);
    }
    ...
    * @param outputPin The pin connected to your piezo buzzer
     * @param pwmChannel The PWM channel you want to use: ESP32 supports 0 - 15
     */
    void setupOutputs(uint8_t outputPin, uint32_t frequency, uint8_t resolution){
      // if(_pinAttached){
      ...
      ledcAttach(_outputPin, _frequency, _resolution);
